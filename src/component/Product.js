...
const Product = ({cartProduct, setCartProduct, cartShow, setCartShow, searchTerm}) => {
    ...
    let {data, isLoading, error} = useFetch("https://fakestoreapi.com/products")
    ...
    return (<div className={`bg-light py-5 ${cartShow && 'mb-260'} `}>
                ...
                {data?.map(item => (<Col sm={6} md={4} lg={3} className='mb-4' key={item.id}>
                        ...
                        <Card className={`h-100`}>
                            <div className='card-image position-relative overflow-hidden'>
                                ...
                                <div
                                    className="d-flex justify-content-between gap-3 start-0 position-absolute w-100 add-price-btn">
                                    ...
                                    <Button
                                        ...
                                        onClick={() => handleAddToCart(item)}>{cartProduct.find(cp => cp.id === item.id)
                            ...
                            <Card.Body>
                                <div className="d-flex flex-column h-100">
                                    ...
                                    <div className='card-rating mb-2 d-flex align-items-center gap-2'>
                                        {[...Array(5)].map((each, index) => (index < Math.floor(item.rating.rate) ?
                                    ...
                                    <div className='mb-3'>
                                        ...
                                        <Card.Text>
                                            {item.description.length > 100 ? `${item.description.slice(0, 100)} ... `
                                                                           : item.description}
                                        </Card.Text>
                                    </div>
                                    {/*ANOTHER DESIGN*/}
